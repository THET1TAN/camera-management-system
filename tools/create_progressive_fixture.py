...
def main():
    ...
    for cid, suffix, gop, options in ((901, 'ts', 48, []), (902, 'ts', 480, []),
        ...
        filename = f'C{cid}.{suffix}'
        counter = (f"drawtext=fontfile='{escaped_font}':text='C{cid}  %{{pts\\:hms}}  frame %{{n}}':"
        ...
        rows.append({'camera_id': cid, 'file': filename, 'start': start, 'end': start+180})
    ...
    print('Created C901 MPEG-TS, C902 long GOP, C903 tail-metadata MP4, C904 faststart MP4.')
    print('Transfer is slowed by 0.3 seconds per 256 KiB. No player was opened.')
    print('Select 12:00:02. The source counter is relative to 12:00:00; audio starts muted.')
