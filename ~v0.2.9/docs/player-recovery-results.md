...
Release preparation for **v0.2.9**, 2026-09-18: **246 tests pass without skips**
from the active source root in **14.051 s**, and the same **246 tests pass without
skips** from the `~v0.2.9` snapshot in **13.814 s**. The snapshot contains the
same 65 public files as the root selection, including sources, assets, tests,
documentation and the RTSP bench. Its playback-option regression uses the
unchanged sibling `~v0.2.8` baseline. Release preparation changes the version
title and documentation, not the playback/recovery implementation tested below.
